...
file_path ='/home/jamal/Downloads/additional_files/umdfaces_batch2_ultraface_new1.csv'
...
if load_net:
    net = Net().to(device)
    checkpoint = torch.load('/home/jamal/Desktop/trained nn/checkpoint.pth.tar',map_location='cpu')
...
# #     
val_index1 = np.random.permutation(val_index)[:100]
val_dataloader = DataLoader(dataset,batch_size=10,sampler = SubsetRandomSampler(test_index))
val_iter = iter(val_dataloader)
predicted_lables = []
real_labels_g = []
predicted_lables_g = []
